                ...
                <div x-show="{{ $loop->iteration }} <= 6 || showAllCoils" x-cloak id="{{ $product->slug }}"
                    ...
                    <div class="p-8 flex-grow flex flex-col justify-between transition-colors duration-300">
                        ...
                        <div class="mt-8 flex items-center justify-between gap-4 border-t border-zinc-200/80 dark:border-zinc-900/80 pt-6 transition-colors duration-300">
                            ...
                            @if($product->stock > 0)
                            <button type="button"
                                @click="addToCart(@js((string) $product->id), @js($product->title), @js((float) $effectivePrice), @js($product->image_path ?: '/images/products/coil.png'), @js(collect(['version', 'diameter', 'resistance', 'resistance_single', 'resistance_dual', 'foot', 'wrap', 'material', 'durability'])->mapWithKeys(fn ($key) => [$key => (string) ($product->specifications[$key] ?? '')])), @js($product->marketplace_urls ?: new \stdClass())); orderModalOpen = true;"
