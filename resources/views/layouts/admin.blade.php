...
<head>
    ...
    <title>@yield('title', 'Admin Dashboard') - {{ $siteSettings->website_name ?? 'Extreme Project' }}</title>
    ...
    <style>
        ...
        .stealth-card:hover {
            ...
            box-shadow: 0 10px 30px -15px rgba(255, 64, 129, 0.1);
        }
        ...
        html.light aside nav a.text-slate-450,
        html.light aside nav a.text-slate-455 {
            color: #71717a !important;
        }

        html.light aside nav a.text-slate-450:hover,
        html.light aside nav a.text-slate-455:hover {
            color: #18181b !important;
        }
        ...
        html.light .admin-logo-light {
            display: block !important;
        }

        html.light .admin-logo-dark {
            display: none !important;
        }
    </style>
    @if($siteSettings && $siteSettings->favicon_path)
    <link rel="icon" type="image/x-icon" href="{{ $siteSettings->favicon_path }}">
    @endif
</head>
            ...
            <div class="flex flex-col flex-grow pt-5 overflow-y-auto">
                <div class="flex items-center justify-between flex-shrink-0 px-6 mb-10 w-full">
                    <span class="text-base font-black tracking-tighter text-slate-100 font-display flex items-center gap-1.5 uppercase">
                        ...
                        <img src="{{ $siteSettings->logo_path }}" alt="Logo" class="h-12 w-auto object-contain admin-logo-dark">
                        @endif
                        @if($siteSettings && ($siteSettings->logo_light_path || $siteSettings->logo_path))
                        <img src="{{ $siteSettings->logo_light_path ?: $siteSettings->logo_path }}" alt="Logo" class="h-12 w-auto object-contain admin-logo-light">
                        @else
                        <span class="w-2 h-2 bg-industrial-orange shadow-[0_0_8px_#FF4081]"></span>
                        @endif
                    </span>
                    <!-- Theme Toggle Button -->
                ...
                <nav class="flex-1 px-4 space-y-2">
                    ...
                    <a href="{{ route('admin.events.index') }}"
                        class="group flex items-center px-4 py-2.5 text-xs font-bold rounded transition-all duration-200 uppercase tracking-wider {{ request()->routeIs('admin.events.*') ? 'bg-industrial-orange/10 text-industrial-orange border-l-2 border-industrial-orange shadow-[inset_4px_0_12px_rgba(255,85,0,0.05)]' : 'text-slate-455 hover:text-slate-205 hover:bg-zinc-900 border-l-2 border-transparent' }}">
                        <svg class="mr-3 h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <a href="{{ route('admin.surveys.index') }}"
                        class="group flex items-center px-4 py-2.5 text-xs font-bold rounded transition-all duration-200 uppercase tracking-wider {{ request()->routeIs('admin.surveys.*') ? 'bg-industrial-orange/10 text-industrial-orange border-l-2 border-industrial-orange shadow-[inset_4px_0_12px_rgba(255,85,0,0.05)]' : 'text-slate-455 hover:text-slate-205 hover:bg-zinc-900 border-l-2 border-transparent' }}">
                        <svg class="mr-3 h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <a href="{{ route('admin.testimonials.index') }}"
                        class="group flex items-center px-4 py-2.5 text-xs font-bold rounded transition-all duration-200 uppercase tracking-wider {{ request()->routeIs('admin.testimonials.*') ? 'bg-industrial-orange/10 text-industrial-orange border-l-2 border-industrial-orange shadow-[inset_4px_0_12px_rgba(255,85,0,0.05)]' : 'text-slate-455 hover:text-slate-205 hover:bg-zinc-900 border-l-2 border-transparent' }}">
                        <svg class="mr-3 h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <a href="{{ route('admin.guides.index') }}"
                        class="group flex items-center px-4 py-2.5 text-xs font-bold rounded transition-all duration-200 uppercase tracking-wider {{ request()->routeIs('admin.guides.*') ? 'bg-industrial-orange/10 text-industrial-orange border-l-2 border-industrial-orange shadow-[inset_4px_0_12px_rgba(255,85,0,0.05)]' : 'text-slate-455 hover:text-slate-205 hover:bg-zinc-900 border-l-2 border-transparent' }}">
                        <svg class="mr-3 h-4 w-4 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="flex-1 flex flex-col md:pl-64">
            <header class="md:hidden flex items-center justify-between h-16 bg-industrial-dark border-b border-industrial-border sticky top-0 z-30 px-4">
                <div class="flex items-center gap-3">
                    <span class="text-base font-black tracking-tighter text-slate-100 font-display flex items-center gap-1.5 uppercase">
                        @if($siteSettings && $siteSettings->logo_path)
                        <img src="{{ $siteSettings->logo_path }}" alt="Logo" class="h-5 w-auto object-contain admin-logo-dark">
                        @endif
                        @if($siteSettings && ($siteSettings->logo_light_path || $siteSettings->logo_path))
                        <img src="{{ $siteSettings->logo_light_path ?: $siteSettings->logo_path }}" alt="Logo" class="h-5 w-auto object-contain admin-logo-light">
                        @else
                        <span class="w-1.5 h-1.5 bg-industrial-orange shadow-[0_0_8px_#FF4081]"></span>
                        @endif
                        {{ $siteSettings->website_name ?? 'EXTREME PROJECT' }}
                    </span>
                    <!-- Theme Toggle Button -->
                    <button @click="theme = (theme === 'dark' ? 'light' : 'dark'); localStorage.setItem('theme', theme); document.documentElement.classList.toggle('light', theme === 'light')"
                        class="p-1 rounded border transition-all focus:outline-none flex items-center justify-center"
            ...
            <div class="fixed inset-y-0 left-0 flex flex-col max-w-xs w-full bg-industrial-black border-r border-industrial-border z-50 md:hidden"
                ...
                <nav class="flex-1 px-4 py-4 space-y-2 overflow-y-auto">
                    ...
                    <a href="{{ route('admin.surveys.index') }}" class="block px-3 py-2.5 rounded text-sm font-bold uppercase tracking-wider {{ request()->routeIs('admin.surveys.*') ? 'bg-industrial-orange/10 text-industrial-orange' : 'text-slate-400 hover:bg-slate-900' }}">
                        Hasil Survei
                    </a>
                    <a href="{{ route('admin.testimonials.index') }}" class="block px-3 py-2.5 rounded text-sm font-bold uppercase tracking-wider {{ request()->routeIs('admin.testimonials.*') ? 'bg-industrial-orange/10 text-industrial-orange' : 'text-slate-400 hover:bg-slate-900' }}">
                        Testimoni
                    </a>
                    <a href="{{ route('admin.guides.index') }}" class="block px-3 py-2.5 rounded text-sm font-bold uppercase tracking-wider {{ request()->routeIs('admin.guides.*') ? 'bg-industrial-orange/10 text-industrial-orange' : 'text-slate-400 hover:bg-slate-900' }}">
                        Panduan Edukasi
                    </a>
